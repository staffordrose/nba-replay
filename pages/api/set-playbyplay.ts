...
import { getGames, getPlayByPlay } from '../../src/lib/nba-stats';

const BASE_PATH = 'public/data';
const SEASON = '2021';
const RESOURCE = 'playbyplay';
  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        const playbyplay = await getPlayByPlay(game.gameId);

        fs.writeFile(
          `${PATH}/${game.gameId}.json`,
          JSON.stringify(playbyplay, null, 2),
          (err: any) => {
            if (err) throw err;
          }
        );
